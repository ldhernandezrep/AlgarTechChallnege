...
class LocationGoogleRepositoryImpl @Inject constructor(val locationDao: LocationDao, val remote: PlaceGoogleService) : LocationGoogleRepositoy {
        ...
        flow {
            when (val response = remote.getLocationByQuery(query,apiKey)) {
                is NetworkResult.NetWorkSuccess -> {
                    ...
                    emit(ResultType.Success(data = response.result.results.map { it.geometry.location.toModel(it.formattedAddress) }))
                }
